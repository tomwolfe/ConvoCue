...
export const useML = () => {
    ...
    // Updated to implement smart context window with key information preservation
    ...
    const summarizeSession = useCallback(() => {
        ...
        const stats = {
            ...
            keyThemes: messagesRef.current.conversationThemes.join(', '),
            keyEntities: messagesRef.current.keyEntities.slice(-10).join(', ')
        };
    ...
    const startNewSession = useCallback(() => {
        ...
        clearTranscript();
        resetBattery();
        // Reset the enhanced context management
        messagesRef.current = {
            conversation: [],
            keyEntities: [],
            conversationThemes: []
    ...
    const processText = useCallback((text) => {
        ...
        const newMessage = { role: 'user', content: `${speakerLabel}: ${text}`, timestamp: Date.now() };
        messagesRef.current.conversation.push(newMessage);

        // Extract key entities and themes from the conversation
        const words = text.toLowerCase().split(/\s+/);
        const newEntities = words.filter(word =>
            AppConfig.personas[persona]?.examples?.some(ex =>
                ex.suggestion.toLowerCase().includes(word)
            ) || word.length > 4 // Consider longer words as potential entities
        );
        messagesRef.current.keyEntities = [...new Set([...messagesRef.current.keyEntities, ...newEntities])];

        // Update conversation themes based on intent
        if (!messagesRef.current.conversationThemes.includes(intent)) {
            messagesRef.current.conversationThemes.push(intent);
        }

        // Maintain a smart sliding window: keep recent messages and important context
        if (messagesRef.current.conversation.length > 10) {
            // Keep the last 6 messages plus important context
            const recentMessages = messagesRef.current.conversation.slice(-6);
            // Add summary of earlier conversation if needed
            const contextSummary = {
                role: 'system',
                content: `Previous conversation themes: ${messagesRef.current.conversationThemes.join(', ')}. Key entities mentioned: ${messagesRef.current.keyEntities.slice(-5).join(', ')}.`
            };
            messagesRef.current.conversation = [contextSummary, ...recentMessages];
        }
        ...
        if (llmWorkerRef.current) {
            llmWorkerRef.current.postMessage({
                ...
                taskId,
                data: {
                    messages: [...messagesRef.current.conversation],
                    context: contextData,
