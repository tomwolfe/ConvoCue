 ...
 *
 * OPTIMIZATION: Simplified state transition logic to reduce complexity and potential race conditions.
 */
...
export class MLStateMachine {
  constructor(maxRetries = 3) {
    ...
    // Add a lock to prevent race conditions during state transitions
  ...
  async transition(transition, contextUpdate = {}) {
    // Prevent concurrent transitions to avoid race conditions
    if (this._transitionLock) {
      // Wait briefly and retry, or return current state if still locked
      await new Promise(resolve => setTimeout(resolve, 10));
      if (this._transitionLock) {
        console.warn(`State transition ${transition} blocked by ongoing transition, returning current state: ${this.state}`);
        return this.state;
      }
    }
    ...
    try {
      ...
      if (transition === ML_TRANSITIONS.RETRY_STT || transition === ML_TRANSITIONS.RETRY_LLM) {
        ...
        // If we've exceeded max retries, force an error state
        if (this.context.retryCount > this.context.maxRetries) {
           console.warn(`Max retries exceeded (${this.context.retryCount}/${this.context.maxRetries}), transitioning to ERROR state`);
           nextState = ML_STATES.ERROR;
        }
      }
      ...
      // Keep only the last 50 transitions to prevent memory bloat
  ...
  /**
   * Get a snapshot of the current state and context for debugging
   */
  getSnapshot() {
    return {
      state: this.state,
      context: { ...this.context },
      isLocked: this._transitionLock
    };
  }
}
