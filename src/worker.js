...
class MLPipeline {
    ...
    async loadSTT(progress_callback) {
        if (!MLPipeline.stt) {
            console.log(`Loading STT model (${AppConfig.models.stt.name}, ${AppConfig.models.stt.dtype}) from Hugging Face...`);
            try {
    ...
    async loadLLM(progress_callback) {
        if (!MLPipeline.llm) {
            console.log(`Loading LLM model (${AppConfig.models.llm.name}, ${AppConfig.models.llm.dtype}) from Hugging Face...`);
            try {
...
const cleanupModels = () => {
    if (MLPipeline.stt) {
        try {
            // Free model resources if available
            if (MLPipeline.stt?.tokenizer?.model?.free) {
                MLPipeline.stt.tokenizer.model.free();
            }
            if (MLPipeline.stt?.processor?.feature_extractor?.free) {
                MLPipeline.stt.processor.feature_extractor.free();
            }
            MLPipeline.stt = null;
    ...
    if (MLPipeline.llm) {
        try {
            // Free model resources if available
            if (MLPipeline.llm?.tokenizer?.free) {
                MLPipeline.llm.tokenizer.free();
            }
            if (MLPipeline.llm?.model?.free) {
                MLPipeline.llm.model.free();
            }
            MLPipeline.llm = null;
            console.log("LLM model cleaned up");
        } catch (e) {
            console.warn("Error cleaning up LLM model:", e);
        }
    }
};
...
self.onmessage = async (event) => {
    ...
    if (memoryInfo && memoryInfo.isHighMemory) {
        ...
        // Optionally send memory warning to main thread
        self.postMessage({
            type: 'memory_warning',
            memoryInfo,
            taskId
