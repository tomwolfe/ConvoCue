...
export const validateSocialSuggestion = (suggestion, context = '') => {
  ...
  // Improved detection: check for meta-talk about these topics

  const isDiscussionContext = /avoid|discuss|talk about|how not to|don'?t do|problem with|negative effect of|impact of|consequences of|understand/.test(context.toLowerCase()) ||

    (context.includes('?') && /manipulate|gaslight|deceive|trick|lie|coerce/.test(context.toLowerCase()));
...
If this is a false positive, please report it in Settings > Feedback.`);
        ...
        trackEthicsEvent('harmful_pattern_blocked', { matchedWord, context, suggestion });
    ...
    If this is a false positive, please report it in Settings > Feedback.`);
        ...
        trackEthicsEvent('manipulative_tactic_blocked', { matchedTactic, context, suggestion });
    ...
    export const promoteEmpathy = (suggestion, detectedEmotion) => {
      ...
      const emotionalStates = ['sadness', 'anger', 'fear', 'frustration'];

    

      if (!suggestion || !emotionalStates.includes(detectedEmotion)) {
      ...
      for (const { pattern, replacement } of DISMISSIVE_PATTERNS) {
        ...
        if (pattern.test(suggestion)) {
          ...
          trackEthicsEvent('empathy_promoted', { emotion: detectedEmotion, pattern: pattern.source, originalSuggestion: suggestion });
