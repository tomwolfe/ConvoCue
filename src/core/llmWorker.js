...
const isWebGPUSupported = typeof navigator !== 'undefined' && navigator.gpu !== undefined;
    ...
    try {
        switch (type) {
            ...
            case 'llm':
                ...
                const { messages, context, instruction, retry, retryWithDifferentApproach } = data;

                const recentIntentsStr = context.recentIntents ? ` Recent intents: ${context.recentIntents}.` : '';
                const approachNote = retryWithDifferentApproach
                    ? '\nPrevious suggestions were not helpful. Try a completely different angle.'
                    : '';
                const systemPrompt = `Role:${context.persona}. Intent:${context.intent}.${recentIntentsStr} Battery:${context.battery}%. Goal:${instruction}.${approachNote} Context: Provide a relevant, concise suggestion based on the conversation history.`;
...
- If exhausted, suggest exit strategies\`\`\n` +
                ...
                const output = await llmPipeline(fullPrompt, {
                    ...
                    callback_function: (tokens) => {
                        self.postMessage({ type: 'llm_token', token: tokens, taskId });
                    },
                });
